...
#    KSD = Keys.initKSD(t=-1)

    def __init__(self):
        self.dmpKSVHdr(csv=1,   t=-1)
        self.KSD = self.initKSD(t=-1)
        self.dmpKSVHdr(csv=1,   t= 1)
        self.dumpKSH(  csv=1)
    ########################################################################################################################################################################################################
    def getKSD(self): return self.KSD
    ...
    def dumpData(self, csv=0):
        ...
        slog(f'END')
    ########################################################################################################################################################################################################
    def dumpKSV(self, csv=0):
        f = 3 if csv else 1
        self.dmpKSVHdr(csv)
        keys = sorted(self.KSD.keys())
        for k in keys:    slog(self.fmtKSK(k, csv), p=0, f=f)
...
#       ns  = [ Notes.name(j, t, 0)                       for j in jz ] # 2 7 9 1 3 6 8 a
    ...
    def dumpKSH(self, csv=0):
        ...
        fs = []  ;  fs.extend(f)   ;  fs.append(y)  ;   fs.extend(s)   ;   slog(f'{v}{fmtl(fs, w=w, d=d, s=m)}', p=p, f=ff)
    ########################################################################################################################################################################################################
    def nic2KS(self, nic, dbg=0):
        KSD, KIM, KIS, KMS, KSK, KST = self.KSD, self.KIM, self.KIS, self.KMS, self.KSK, self.KST
        if dbg: self.dumpKSV()   ;   self.dumpKSH()   ;   self.dumpNic(nic)
        ...
        i   = KSD[k][KIM][KSK]
        ns  = KSD[k][KMS]
        slog(self.fmtKSK(k))      if dbg else None
        return k, nt, n, i, ns, Scales.majIs(i)

    def dumpNic(self, nic): #fix me
