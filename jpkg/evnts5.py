...
def on_text_motion(tobj, motion, dbg=1):
    ...
    if   isNumLck(keys):                           msg =             f'NUMLOCK(        {motion})'   ;   slog(msg)   ;   pygwink.MOD_NUMLOCK = 0
    if   isCtlAltShf(keys):                        msg =             f'@&^(            {motion})'   ;   slog(msg) #  ;   self.quit(msg)
    elif isCtlAlt(keys): # keys[k.MOD_CTRL & k.MOD_ALT]:
...
#   elif kbk == 'BACKSLASH':                 tobj.setCHVMode(    '  BACKSLASH', ARPG,   RARROW, DARROW)
    elif kbk == 'D' and isAltShf(mods):      tobj.flipBGC(     '&^D')
    elif kbk == 'D' and isAlt(   mods):      tobj.flipBGC(     '& D')
    elif kbk == 'N' and isAltShf(mods):      tobj.setn_cmd(    '&^N', txt=Z)
    elif kbk == 'N' and isAlt(   mods):      tobj.setn_cmd(    '& N', txt=Z)
    elif kbk == 'P' and isAltShf(mods):      tobj.flipPage(    '&^P', 1)
    elif kbk == 'P' and isAlt(   mods):      tobj.flipPage(    '& P', -1)
    elif kbk == 'R' and isAltShf(mods):      tobj.rotateSprite('&^R', hcurs[0], -1)
    elif kbk == 'R' and isAlt(   mods):      tobj.rotateSprite('& R', hcurs[0],  1)
    elif kbk == 'Z' and isAltShf(mods):      tobj.RESIZE = not tobj.RESIZE   ;  tobj.resizeTniks(dbg=1)
    elif kbk == 'Z' and isAlt(   mods):                                         tobj.resizeTniks(dbg=1)
    ####################################################################################################################################################################################################
    elif kbk == 'B' and isAltShf(mods):      tobj.setFontParam(BOLD,      not fontBold,   'fontBold')
    elif kbk == 'B' and isAlt(   mods):      tobj.setFontParam(BOLD,      not fontBold,   'fontBold')
    elif kbk == 'C' and isAltShf(mods):      tobj.setFontParam(COLOR,         1,          'clrIdx')
    elif kbk == 'C' and isAlt(   mods):      tobj.setFontParam(COLOR,        -1,          'clrIdx')
    elif kbk == 'I' and isAltShf(mods):      tobj.setFontParam(ITALIC,    not fontItalic, 'fontItalic')
    elif kbk == 'I' and isAlt(   mods):      tobj.setFontParam(ITALIC,    not fontItalic, 'fontItalic')
    elif kbk == 'A' and isAltShf(mods):      tobj.setFontParam(FONT_NAME,     1,          'fontNameIdx')
    elif kbk == 'A' and isAlt(   mods):      tobj.setFontParam(FONT_NAME,    -1,          'fontNameIdx')
    elif kbk == 'S' and isAltShf(mods):      tobj.setFontParam(FONT_SIZE,     33 / 32,    'fontSize')
