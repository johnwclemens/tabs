...
#        self.catPath      = str(BASE_PATH / 'cats' / BASE_NAME) + '.cat'
        ...
        self.GEN_DATA     = 0  ;  self.MULTI_LINE  = 1  ;  self.ORDER_GROUP  = 1  ;  self.RD_STDOUT  = 0  ;  self.RESIZE      = 1
        self.SNAPS        = 1  ;  self.SPRITES     = 0  ;  self.SUBPIX       = 1  ;  self.TEST       = 0  ;  self.VERBOSE     = 0  ;  self.TIDS    = 0
    ...
    def _reinit(self):
        ...
        self.rowLs, self.qcols, self.hcurs, self.views = [], [], [], []  ;  self.C = [self.rowLs, self.qcols, self.hcurs, self.views]
        self.snums, self.snams, self.capos, self.isvis = [], [], [], []  ;  self.D = [self.snums, self.snams, self.capos, self.isvis]
    ...
    def _initColors(self):
        ...
        j = R  ;  k[j] = i(j, RED,  0, 17, RED, 17, 17) if a else i(j, BLU,  0, 17, BLU, 17, 17) if b else i(j, BLU,  0,  4, BLU, 17,  4) if c else i(j, BLU,  0, 17, BLU, 17, 17) if d else None
        ...
        j = H  ;  k[j] = i(j, PNK, 17, 17, PNK, 17, 17) if a else i(j, PNK, 17, 10, PNK, 17, 17) if b else i(j, PNK, 17, 17, PNK, 17, 17) if c else i(j, PNK, 17, 17, PNK, 17, 17) if d else None
        j = V  ;  k[j] = i(j, VLT,  0,  0, VLT, 17, 17) if a else i(j, VLT,  0,  0, VLT, 17, 17) if b else i(j, VLT,  0,  0, VLT, 17, 17) if c else i(j, VLT,  0,  0, VLT, 17, 17) if d else None
        j = O  ;  k[j] = i(j, PNK,  0,  0, PNK, 17, 17) if a else i(j, PNK,  0,  0, PNK, 17, 17) if b else i(j, PNK,  0,  0, PNK, 17, 17) if c else i(j, PNK,  0,  0, PNK, 17, 17) if d else None
...
#        self.data = self.A_transposeData(dump=dbg) if self.TRANSPOSE_A else self.transposeDataDump() if dbg else self.OLD_transposeData()
    ...
    def _initTniks(self):
        self.ssl()      ;   self.smap = {}
        self.dumpAxy()  ;   self.dumpAXY()
        [ self.isvis.append(list()) for _ in range(len(JTEXTS)) ]
        self.createTniks()
    ...
    def _initGroups(self):
        hdrA    = [P, L, S, C,  T, N, I, K,  R, Q, H, V,  O, A, D, W]
    ...
    ####################################################################################################################################################################################################
    def test(self, j=10):
        hdr = f' cc  [ tpb  tpp tpl tpc] return [p l  c  t]'
        self.dumpTniksPfx(f'BGN {j=} test', r=0)
        self.log(hdr)
        for i in range(len(self.tabls) * 2): self.cc2plct(i)
        self.log(hdr)
        self.dumpTniksSfx(f'BGN {j=} test')
    ####################################################################################################################################################################################################
    ...
    def setJ(    self, j, n):         self.J1[j] = n    ;   self.J2[j] += 1          ;   self.J1[-1] += 1   ;  self.J2[-1] += 1  ;  return j #  ;  self.J1[-2] += 1 if self.isV(j) else 0  ;  self.J2[-2] += 1 if self.isV(j) else 0
    ...
    ####################################################################################################################################################################################################
    def jsum(self, a=1): return [ _ + a if j <= H and self.J2[j] else _ if j == len(self.J2) else 0 for j, _ in enumerate(self.J1) ]
    ...
    def fmtLE( self, w=None, d=1):  w = w if w is not None else JFMT  ;  d1 = "" if not d else "["  ;  d2 ="" if not d else "]"  ;  return f'{d1}{util.fmtl(self.lenE(), w=w, d1="", d2="")} {sum(self.lenE()):4}{d2}'
    ####################################################################################################################################################################################################
...
#    def fplct(self):                          return f'{util.fmtl(self.j2())}'
    ...
    def fmtD(self, data=None,       d='x'):  l = list(map(str, self.dl(data)))               ;   return f'({d.join(l)})'
    def fmtI(self,                  d='x'):  l = list(map(str, self.i))   ;   del l[S:S+1]   ;   return f'({d.join(l)})'
    def fmtWH(self, w=None, h=None, d='x'):  w = w if w is not None else self.width  ;  h = h if h is not None else self.height  ;  return f'({w}{d}{h})'
...
#    def dumpObj( obj,  name, why='', file=None): util.slog(f'{why} {name} ObjId {id(obj):x} {type(obj)}', file=file)
    ...
    def dumpGeom(self, why1='', why2=''):        self.log(f'{why1:4} {self.fmtD()} {self.fmti()} {util.fmtl(self.ss2sl()):9} {self.LL} {util.fmtl(self.zz2sl()):5} {self.fmtn()} {len(self.idmap)} {why2}')
    ...
    def _initntp(self, dbg=1):
        ncp = self.ncp(dbg=dbg)  ;  self.cpb, self.cpp, self.cpl           = ncp
        ntp = self.ntp()         ;  self.tpb, self.tpp, self.tpl, self.tpc = ntp
        if dbg: self.log(f'{self.tpb=} {self.tpp=} {self.tpl=} {self.tpc=} ntp={util.fmtl(ntp)}')

    def ncp(self, dbg=0): ntp = self.ntp(dbg=dbg)  ;  ncp = [ ntp[i] for i in range(len(ntp)-1) ]  ;  self.log(util.fmtl(ncp)) if dbg else None  ;  return ncp

    def ntp(self, d2=1, dbg=0):
        d = 'd2' if d2 else '  '
        n = list(self.n)     ;   self.log(f'           n={ self.fmtn("", n)}') if dbg else None
        if d2:   del n[2]    ;   self.log(f'         n{d}={self.fmtn("", n)}') if dbg else None
        n.reverse()          ;   self.log(f'       {d}Rev={self.fmtn("", n)}') if dbg else None
        n = self.accProd(n)  ;   self.log(f'   n{d}RevPrd={util.fmtl(n)}')     if dbg else None
        n.reverse()          ;   self.log(f'n{d}RevPrdRev={util.fmtl(n)}')     if dbg else None
        return n
    @staticmethod
    ...
    def dumpStruct(self, why='', dbg=1, dbg2=1):
        ...
        if dbg:     self.dumpTniksC(f'{why}C')
...
#                if ll:  llt = f'{JTEXTS[L]} {l+1}'  ;  llab = f'{llt:{i+1}}'  ;  self.log(f'{Z*i}{llab}', pfx=0)
    ...
    def addZZs(self, how, ii):
        ...
        for p in range(np):
            ...
            for l in range(nl):
                self.setJdump(L, l, why2) # l
                for s, s2 in enumerate(self.ss2sl()):
                    self.setJdump(S, s2, why2) # s2
                    for c in zz:
                        if   c == ii:   self.addZZ(p, l, s, c, c, why)
    ...
    def NEW__addZZs(self, how, ii):
        ...
        self.dumpTniksSfx(why)
    ####################################################################################################################################################################################################
    def addZZ(self, p, l, s, c, ii, why):
        np, nl, ns, nc, nt = self.n    ;   zzl = self.zzl()   ;   nc += zzl
        z1 = self.z1(c)   ;   z2 = self.z2(c)
        self.log(f'ii={ii} j={C} {JTEXTS[C]:4} nc={nc}      ltl={len(self.colms)} plsc =[{p} {l} {s} {c}  ] J1={self.fmtJ1(0, 1)} J2={self.fmtJ2(0, 1)} z1={z1} z2={z2}', so=1)
        for colm in self.g_createTniks(None, C, self.sects[s], ii=ii):
            c2 = c + nc*(s + ns*(l + nl*p))
            self.colms.insert(c2, colm)
            if self.LL and self.isV() and not s: # C
                n, _, x, y, w, h = self.geom(C, self.rowLs[l], 1)
                lrCol = self.createLL(None, l, c, x, y, w, h, why)
                self.qcols.insert(c2, lrCol)
            ...
            for t, _ in enumerate(self.g_createTniks(self.tabls, T, colm, ii=s, why=why)):
...
#                                self.setJdump(j, t, why2)
    ...
    def addTTs(self, how, ii):
        ...
        for p in range(np):
            self.setJdump(P, p, why2) #, p
            for l in range(nl):
                self.setJdump(L, l, why2)   ;   ss = self.ss2sl()
    ...
    def addLLs(self, how):
        ...
        self.dumpTniksSfx(why)
    ####################################################################################################################################################################################################
    def OLD__addPage(self, how, ins=None, dbg=1):
        ...
        self.data = self.A_transposeData(dump=dbg) if self.TRANSPOSE_A else self.OLD_transposeData()
        self.dumpTniksPfx(how, r=0)   ;   pi = len(self.pages)
        self.J1[P], self.J1[L], self.J1[S], self.J1[C], self.J1[T] = 0, 0, 0, 0, 0
        n, ii, x, y, w, h =    self.geom(V, n=1, i=1, dbg=1)   ;   kk = self.cci(P, 0, kl)
        self.newC += 1  ;  why2 = f'New{self.newC}'  ;  why = why2
        page = self.createTnik(self.pages,   pi, P, x, y, w, h, kk, kl, why=why, dbg=1)
        for line in            self.g_createTniks(self.lines,  L, page, why=why):
            for sect in        self.g_createTniks(self.sects,  S, line, why=why):
                for colm in    self.g_createTniks(self.colms,  C, sect, why=why):
                    for _ in   self.g_createTniks(self.tabls,  T, colm, why=why):
                        pass
...
#        if ins is not None: self.togglePage(how)
        if ins is not None: self.toggleVisible()
        self.n[P] += 1   ;   kl = self.k[P]
        data = [ [ self.tblankRow for _ in range(nt) ] for _ in range(nl) ]
        self.data = self.A_transposeData(dump=dbg) if self.TRANSPOSE_A else self.OLD_transposeData()
        self.data.append(data) if ins is None else self.data.insert(ins, data)
...
#        self.LL = oldLL
    ...
    def splitV(self, p, a, dbg=0):
        if   type(p) is LBL: p.y, p.height, g     = self.splitV1(p.y, p.height, a)                  ;  self.log(f'{p.y=:6.2f} {p.height=:6.2f} {a=} {g=:6.2f}', so=1)  if dbg else None  ;   return p
        elif type(p) is SPR: p.y, h, g, p.scale_y = self.splitV2(p.y, p.height, a, p.image.height)  ;  self.log(f'{p.y=:6.2f} {p.scale_y=:6.4f} {a=} {h=:6.2f}', so=1) if dbg else None  ;   return p
...
#    def sprite2LabelPos(self, x, y, w, h, dbg=0): x0 = x  ;  y0 = y  ;  x += w/2  ;  y -= h/2  ;  self.log(f'{x0=:6.2f} {y0=:6.2f}, {w/2=:6.2f} {-h/2=:6.2f}, {x=:6.2f} {y=:6.2f} {self.p0x=:6.2f} {self.p0y=:6.2f}', so=1) if dbg else None  ;  return x, y
    ####################################################################################################################################################################################################
    def OLD__isV(self, j=None, dbg=0): #          j = j if j is not None else P
        if   j is None:                          j = P
        if   j <= T and self.J2[j] == self.i[j]: v = 1
        elif j <= K and self.J2[P] == self.i[P]: v = 1
        elif j == H or j == R or j == Q:         v = 1
        else:                                    v = 0
        if dbg:  why = f'{v=}'  ;  self.log(f'{self.fmtJText(j, why)} {self.J2[j]=} {self.i[j]=} {self.fmti()} {v=}', file=sys.stdout)
        return v
...
#        if   j <= K and self.J1[P] == self.i[P] - 1: v = 1
    ...
    def j2plct(self, p=None, l=None, c=None, t=None): #        return p2 % np, l2 % nl, c2 % nc, t2 % nt
        ...
        rt = t2 % nt
        return rp, rl, rc, rt
    def OLD_2_j2plct(self, p=None, l=None, c=None, t=None):
        np, nl, ns, nc, nt = self.n   ;   n = 0
        if p is None: p = self.J1[P]
        if l is None: l = self.J1[L]
        if c is None: c = self.J1[C]
...
#        rp = p2 % np
        ...
        rt = t2 % nt
        return p2, rl, rc, rt
    def NEW__j2plct(self, p=None, l=None, c=None, t=None):
    ...
    def tnikInfo(self, p, l, s, c, t=None, why='', dbg=0):
        ...
        if t is None:
            if   s == TT:  tlist, j = (self.snums, O) if exp1 else (self.capos, D) if exp2 else (self.tabls, T)
            elif s == NN:  tlist, j = (self.snams, A) if exp1 else (self.capos, D) if exp2 else (self.notes, N)
            elif s == II:  tlist, j = (self.snums, O) if exp1 else (self.capos, D) if exp2 else (self.ikeys, I)
            ...
            if   s == TT:  tlist, j, k, txt = (self.snums, O, kO, self.sobj.stringNumbs[t]) if exp1 else (self.capos, D, kD, self.sobj.stringCapo[t]) if exp2 else (self.tabls, T, kT, tab)
            ...
            elif s == II:  tlist, j, k, txt = (self.snums, O, kO, self.sobj.stringNumbs[t]) if exp1 else (self.capos, D, kD, self.sobj.stringCapo[t]) if exp2 else (self.ikeys, I, kI, tab)
...
#        if j is None or j < 0 or j >= len(JTEXTS): msg = f'ERROR j is BAD {j=} {len(JTEXTS)=} {n=} {i=} {type(p)=}'  ;  self.log(msg)  ;  self.quit(msg)
        n = n if n is not None else self.n[j]    ;  n += self.zzl() if j == C or j == R else 0
    ...
    def createLLs(self, p, pi, why, dbg=1, dbg2=1):
        ...
        for c in range(nc):
            self.createLL(self.qcols, pi, c, xc, yc, wc, hc, why)
        p = self.splitV(p, n, dbg=dbg2)
        self.dumpTnik(p, L, why=why)
        return p
    ...
    def resizeLLs(self, p, why, dbg=1, dbg2=1):
        ...
        nc, ic, xc, yc, wc, hc = self.geom(Q, lrow, self.n[C], self.i[C], dbg=dbg2)  # ;    xc0 = xc
        for c in range(nc):
            self.resizeTnik(self.qcols, self.J2[Q], Q, xc + c*wc, yc, wc, hc, why, dbg=dbg)
        p = self.splitV(p, n, dbg=dbg2)
        return p
    ####################################################################################################################################################################################################
    def OLD__createTniks(self, dbg=1):
        self.newC += 1  ;  why2 = f'New{self.newC}'  ;  why = why2
        self.dumpTniksPfx(why)   ;   view = None   ;   i = 0
#        if self.VIEWS: # _, _, x, y, w, h = self.geom(V, n=1, i=1, dbg=1)  ;  kk = 0  ;  kl = self.k[V] # view = self.createTnik(self.views, 0, V, x, y, w, h, kk, kl, why=why, dbg=1)
        for i, page in enumerate(self.g_createTniks(self.pages, P, view, why=why, v=0 if i else 1)): # pass
            for line in          self.g_createTniks(self.lines, L, page, why=why, v=0 if i else 1): # pass
                for sect in      self.g_createTniks(self.sects, S, line, why=why, v=0 if i else 1): # pass
                    for colm in  self.g_createTniks(self.colms, C, sect, why=why, v=0 if i else 1): # pass
                        for _ in self.g_createTniks(self.tabls, T, colm, why=why, v=0 if i else 1): pass
        if self.tabls:  self.createCursor(why)
        self.dumpTniksSfx(why)
        if dbg:         self.dumpStruct(why2)
    def OLD__g_createTniks(self, tlist, j, pt, ii=-1, why='', v=0, dbg=1, dbg2=1):
        ...
        for i in range(n):
            ...
            tnik = self.createTnik(tlist2, i if ii < 0 else ii, j2, x2, y2, w, h, kk, kl, why=why, t=text, v=v, dbg=dbg)
            yield tnik
...
#        if self.VIEWS: # _, _, x, y, w, h = self.geom(V, n=1, i=1, dbg=1)  ;  kk = 0  ;  kl = self.k[V] # view = self.createTnik(self.views, 0, V, x, y, w, h, kk, kl, why=why, dbg=1)
        ...
        if dbg:         self.dumpStruct(why2)
    ####################################################################################################################################################################################################
    def g_createTniks(self, tlist, j, pt, ii=None, why='', v=0, dbg=1, dbg2=1):
        n, _, x, y, w, h = self.geom(j, pt, dbg=dbg2)   ;   text = ''   ;    kl = self.k[j]    ;  tlist2 = tlist   ;   p, l, c, _ = self.j2plct()
        n                = n if ii is None else 1       ;   j2 = j   ;   i3 = 0   ;   x2 = x   ;   y2 = y   ;   np, nl = self.dl()[0], self.dl()[1]
        for i in range(n):
            ...
            if   j == C:                                x2 = x + i2 * w
                ...
                if   j == S:                             _ = self.ss2sl()[i2]  ;  self.SS.add(_)
                    ...
                    s = self.ss2sl()[self.J1[S]] # if ii is None else ii
                    imap = self.getImap(p, l, c)  if s >= II else []
                    tlist2, j2, kl, tobj = self.tnikInfo(p, l, s, c, i2, why)
                    if   s == TT:                     text = tobj
                    elif s == NN:                     text = tobj if j2 > K else self.sobj.tab2nn(tobj, i2) if self.sobj.isFret(tobj) else self.tblank
                    elif s == II:                     text = self.imap2ikey( tobj, imap, i3, j2)   ;   i3 += 1 if text != self.tblank else 0
                    elif s == KK:                     text = self.imap2Chord(tobj, imap, i2,  j2)
...
#        v = v           if v is not None else self.isV(j) # v = self.isV(j) if v is None else v
        ...
        if j == H or (self.SPRITES and (j < T or j == R)):
            ...
            elif j == Q:       k = self.k[j] if (i+1) % 10 else self.k[R]   ;   self._setTNIKStyle(tnik, k, NORMAL_STYLE)
        tnik.visible = v   ;   self.isvis[j].append(v)
        if    tlist is not None:         tlist.append(tnik)
        else:                            msg = f'ERROR tlist is None can\'t add tnik: {self.fmtJText(j, why)}'  ;  self.log(msg)  ;  self.quit(msg)
        key = self.idmapkey(j)    ;      self.idmap[key] = (tnik, j, i)   ;   self.dumpTnik(tnik, j, why) if dbg else None
        if self.LL and j == L: # and v
            if not self.rowLs or len(self.rowLs) < self.n[L]: tnik = self.createLLs(tnik, i, why)
            else:                                             tnik = self.splitV(tnik, self.m(), dbg=dbg)
        return tnik
    ####################################################################################################################################################################################################
...
#            view = self.resizeTnik(self.views, 0, V, x, y, w, h, dbg=1)
        ...
        if self.RESIZE and self.cursor: self.resizeCursor(why)
        self.dumpTniksSfx(why)
        if dbg:         self.dumpStruct(why2)
    ####################################################################################################################################################################################################
    def g_resizeTniks(self, tlist, j, pt=None, why='', dbg=1, dbg2=1):
        ...
        for i in range(n):
            if   j == C:                x2 = x + i * w
                ...
                if   j != P:            y2 = y - i * h
                if   j == L and self.J2[L] >= lp * ll: msg = f'WARN MAX Line {self.J2[L]=} >= {lp=} * {ll=}'     ;   self.log(msg)  ;  self.quit(msg) # ;  yield None
                elif j >= T:
...
#            if not self.rowLs or len(self.rowLs) < self.n[L]:
    ...
    def dumpTniksPfx(self, why='', h=1, r=1):
        ...
        self.resetJ(why) if r else self.dumpJs(why, w=None)  if self.J1 and self.J2 else self.quit(f'ERROR No Js {len(self.J1)=} {len(self.J2)=}')
    ...
    def fTnikHdr(self, spr=0):
        tid  = ' TId  Identity  ' if self.TIDS else ' Tid'  ;  wnc = ' Why  Name  Cnt'    ;   tsv = 'Txt fSz V'  ;  j = 'P  L  S   C    T    N    I    K  R   Q H V  O  A   D    W  TId'
        xywh = '    X       Y       W       H  '            ;  rgb = ' Red Grn Blu Opc '  ;  rgbM = ' M     Mx    My  ' if spr else rgb
        sfx  = ' Iax  Iay      Grp        pGrp' if spr else 'cw  ch v a x y dpi B I Font Name'
        return f'{tid} {wnc} {tsv} {xywh} {rgb} G {rgbM} {j} {sfx}'
    ...
    def dumpTnik(self, t=None, j=None, why=''):
        ...
        msg1 = self.fid(t, j, why)   ;   msg3 = f'{self.ftvis(t)} {xywh}'   ;   msg4 = f'{fc} {g} {bc} {self.fmtJ2()}'
        self.log(f'{msg1} {msg2} {msg3} {msg4} {msg5}', pfx=0)
    ####################################################################################################################################################################################################
    ...
    def dumpTniksB(self, why=''):
        np, nl, ns, nc, nt = self.n  ;  tpc = self.tpc  ;  tpl = self.tpl  ;  tpp = self.tpp
        self.dumpTniksPfx(why)
        for p in range(np):
                ...
                for s, s2 in enumerate(self.ss2sl()):
                    ...
                    for c in range(nc):
                        ...
                        for t in range(nt):
                            ...
                            self.setJdump(j, t + c*tpc + l*tpl + p*tpp, why)
        self.setJdump(H, 0, why)
        self.dumpTniksSfx(why)
    ####################################################################################################################################################################################################
    def dumpTniksC(self, why=''):
        self.dumpTniksPfx(why)
        consume(consume(self.setJdump(j, i, why) for i in range(len(self.E[j]))) for j in range(len(self.E)) if j < W)
        self.dumpTniksSfx(why)
    ...
    def moveCursor(self, ss=0, why='', dbg=1):
        ...
        old = self.cc  ;  self.resizeCursor(why, dbg=dbg)
        if dbg:           self.log(f'{old=} {self.cc=}')
        if self.LL:       self.setLLStyle(self.cc, CURRENT_STYLE)
    ####################################################################################################################################################################################################
    def cc2xywh(self, dbg=1):
        ...
        if dbg: self.log(f'{cc=:3} {self.ftxywh(t)} {t.text=} {cw=} {ch=} {ca}', file=sys.stdout)
    ...
    def plct2cc(self, p, l, c, t, dbg=0):
        cc = p * self.tpp + l * self.tpl + c * self.tpc + t   ;   lenT = len(self.tabls)   ;   ccm = cc % lenT if lenT else -1
        if dbg: self.log(f'plct({p} {l} {c} {t}) {self.tpp} {self.tpl} {self.tpc} ({p*self.tpp} +{l*self.tpl} +{c*self.tpc} +{t}) % {lenT} return {ccm}')
        return ccm

    def cc2plct(self, cc, dbg=0):
        tpb, tpp, tpl, tpc = self.ntp()  ;  np, nl, ns, nc, nt = self.n
        p =  cc // tpp % np
        l =  cc // tpl % nl
        c =  cc // tpc % nc
        t =  cc %  tpc
        if dbg: self.log(f'{cc:4} [{tpb:4} {tpp:4} {tpl:3} {tpc:3}] return [{p} {l} {c:3} {t}]')
        return p, l, c, t
...
#    def cn2plct(self, cn):
    ...
    def on_mouse_release(self, x, y, button, modifiers, dbg=1):
        ...
        if dbg: self.log(f'BGN button={button} modifiers={modifiers} txt={self.tabls[self.cc].text}', file=file)
...
#    def isEH(t): return 1 if t == '#' or t == 'b' else 0
    ...
    def setLLStyle(self, cc, style, dbg=0):
        if not self.LL or not self.qcols: msg = f'SKIP {self.LL=} {len(self.qcols)=}'  ;  self.log(msg)  ;  self.quit(msg)
        p, l, c, t = self.cc2plct(cc)
        bold, italic = 0, 0   ;   np, nl, ns, nc, nt = self.n
        i = c + l * nc if self.qcols else 0
...
#        d = self.qcols[i].document # #
#        d.set_style(0, len(d.text), {fc: self.llcolor(i, Q)[fs], bc: self.llcolor(i, Q)[bs]}) # #
        self.setTC(self.qcols[i], self.llcolor(i, Q)[fs], self.llcolor(i, Q)[bs])
        self.qcols[i].bold   = bold
        self.qcols[i].italic = italic
...
#                self.moveTo(how, p0, l0, c0, t0)  ;  cc = self.cursorCol()  ;  self.log(f'AFT {cc0=} {p0=} {l0=} {c0=} {t0=} {cc=}')
    ...
    def togglePage(self, how, dp=1, dbg=1):
        ...
        self.toggleVisible()
        self.dumpVisible()
        if self.SNAPS and dbg: self.regSnap(how, f'TPg{self.i[P]}')
        self.resizeTniks(dbg)
        self.dumpGeom('END', f'{how} {dp=:2} {self.i[P]=}')

    def toggleVisible(self, p=None, dbg=1):
        p = p if p is not None else self.j()[P]
        np, nl, ns, nc, nt = self.n             ;   self.pages[ p].visible = not self.pages[ p].visible
        for l in range(nl):
            l2 = l + p*nl                       ;   self.lines[l2].visible = not self.lines[l2].visible   ;   vl = []
            for s, ss in enumerate(self.ss2sl()):
                s2 = s + l*ns                   ;   self.sects[s2].visible = not self.sects[s2].visible
                for c in range(nc):
                    c2 = c + s*nc               ;   self.colms[c2].visible = not self.colms[c2].visible
                    for t in range(nt):
                        t2 = t + c*nt + l*self.tpl + p*self.tpp
                        tnik = self.B[ss][t2]   ;             tnik.visible = not tnik.visible   ;   v = int(tnik.visible)
                        if dbg and self.VERBOSE:    self.log(f'{v} {p=}, {l=} {l2=}, {s=} {s2=} {ss=}, {c=} {c2=}, {t=} {t2=}')
                        if dbg:                     vl.append(f'{v}')
                if dbg:                             vl.append(' ')
            if dbg:                                 self.log(f'{"".join(vl)}', pfx=0)

    def OLD__dumpVisible(self, h=0):
...
#            self.log(f'{j:2} {JTEXTS[j]}{n:4}/{l} {v}', pfx=0)
    ####################################################################################################################################################################################################
...
JTEXTS = ['Page',  'Line',  'Sect',  'Colm',  'Tabl',  'Note',  'IKey',  'Kord',  'RowL',  'QCol',  'HCrs',  'View',  'ONum',  'ANam',  'DCpo',  'WVis'  'TNIK']
jTEXTS = ['pages', 'lines', 'sects', 'colms', 'tabls', 'notes', 'ikeys', 'Kords', 'rowls', 'qcols', 'hcsrs', 'views', 'onums', 'anams', 'Dcpos', 'wviss', 'tniks']
JFMT   = [  1,       2,       2,       3,       4,       4,       4,       4,       2,       3,       1,       1,       2,       2,       3,       4,       4]
