...
#        self.data = self.A_transposeData(dump=dbg) if self.TRANSPOSE_A else self.transposeDataDump() if dbg else self.OLD_transposeData()
    ...
    def fmtLE( self, w=None, d=1):  w = w if w is not None else JFMT  ;  d1 = "" if not d else "["  ;  d2 ="" if not d else "]"  ;  return f'{d1}{util.fmtl(self.lenE(), w=w, d1="", d2="")} {sum(self.lenE())}{d2}'
    ####################################################################################################################################################################################################
    ...
    def addZZ(self, p, l, s, c, ii):
        ...
        for col in self.g_createTniks(None, C, self.sects[s], ii=ii):
            ...
            if self.LL and self.isV(C) and not s:
                ...
                lrCol = self.addLL(None, l, c, x, y, w, h)
...
#    def sprite2LabelPos(self, x, y, w, h, dbg=0): x0 = x  ;  y0 = y  ;  x += w/2  ;  y -= h/2  ;  self.log(f'{x0=:6.2f} {y0=:6.2f}, {w/2=:6.2f} {-h/2=:6.2f}, {x=:6.2f} {y=:6.2f} {self.p0x=:6.2f} {self.p0y=:6.2f}', so=1) if dbg else None  ;  return x, y
    ...
    def createLL(self, p, pi, dbg=1, dbg2=1):
        ...
        for c in range(nc):
            self.addLL(self.lcols, pi, c, xc, yc, wc, hc)
        self.splitV(p, n, dbg=dbg2)
        self.dumpTnik(p, L, why='Upd')
        return p

    def addLL(self, tlist, l, c, x, y, w, h, dbg=1):
    ...
    def createTnik(self, tlist, i, j, x, y, w, h, kk, kl, why='New', t='', v=None, g=None, dbg=0):
        ...
        if self.LL and j == L:       tnik = self.createLL(tnik, i)
        return tnik
    ####################################################################################################################################################################################################
    @staticmethod
    ...
    def resizeTnik(self, tlist, i, j, x, y, w, h, why='Upd', v=None, dbg=1):
        ...
        if self.LL and j == L:                tnik = self.resizeLL(tnik)
