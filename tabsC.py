...
#        self.catPath      = str(BASE_PATH / 'cats' / BASE_NAME) + '.cat'
        ...
        self.SNAPS        = 1  ;  self.SPRITES     = 0  ;  self.SUBPIX       = 1  ;  self.TEST       = 1  ;  self.VERBOSE      = 0  ;  self.TIDS    = 0
...
#        self.data = self.A_transposeData(dump=dbg) if self.TRANSPOSE_A else self.transposeDataDump() if dbg else self.OLD_transposeData()
    ...
    def setJ(self, j, n):             self.J1[j] = n    ;   self.J2[j] += 1  ;  self.J1[-1] += 1  ;  self.J2[-1] += 1  ;  self.nvis += 1 if self.isV(j) else 0  ;  self.visib[j].append(self.isV(j))
#    def setJ(    self, j, n):         self.J1[j] = n    ;   self.J2[j] += 1  ;  self.J1[-1] += 1  ;  self.J2[-1] += 1 # ;  self.J1[-2] += 1 if self.isV(j) else 0  ;  self.J2[-2] += 1 if self.isV(j) else 0
    ...
    def fplc(  self, p=None, l=None,         c=None):         j = self.J1  ;  p = j[P] if p is None else p  ;  l = j[L] if l is None else l  ;  c = j[C] if c is None else c  ;  return f'[{p} {l} {c:2}]'
    def fplct( self, p=None, l=None,         c=None, t=None): j = self.J1  ;  p = j[P] if p is None else p  ;  l = j[L] if l is None else l  ;  c = j[C] if c is None else c  ;  t = j[T] if t is None else t  ;  return f'[{p} {l} {c:2} {t}]'
    def fplsc( self, p=None, l=None, s=None, c=None):         j = self.J1  ;  p = j[P] if p is None else p  ;  l = j[L] if l is None else l  ;  s = j[S] if s is None else s  ;  c = j[C] if c is None else c  ;  return f'[{p} {l} {s} {c:2}]'
...
#    def fmtPos(self):                         plct = self.j2()   ;  cc = self.plct2cc(*plct)   ;   cn = self.cc2cn(cc)   ;  return f'{util.fmtl(plct)} {cc:3} {cn:2}]'
    def fmtPos(self):                         plct, plsct = self.j2(), self.j()   ;  cc = self.plct2cc(*plct)   ;   cn = self.cc2cn(cc)   ;  return f'{self.fplsct()} {cc:3} {cn:2}]'
...
#    def dumpObj( obj,  name, why='', file=None): util.slog(f'{why} {name} ObjId {id(obj):x} {type(obj)}', file=file)
    ...
    def dumpStruct(self, why='', dbg=1, dbg2=0):
        self.dumpFont(why)
        self.log(f'{self.fmtn()}  ntp={util.fmtl(self.ntp(dbg=1, dbg2=1))}')
        self.dumpVisible()
        self.dumpIdmKeys()
        self.dumpVisible2()
...
#        self.dumpTniksPfx(why1)
        for p in range(np): #            self.setJdump(P, p, why2)
            self.pages[p] = self.splitH(self.pages[p], self.n[C] + self.zzl())
        self.resizeTniks(dbg=1)
#            if self.isV() and 0:
#                for l in range(nl): #                    self.setJdump(L, l, why2)
#                    self.g_resizeTniks(self.lines, L, None, why=why2, dbg=1)
#                    for s in range(self.ssl()): #                        self.setJdump(S, s, why2)
#                        s += l * ns
...
#        self.dumpTniksSfx(why1)
    ...
    def addZZ(self, p, l, s, c, why):
        ...
        for zc in self.g_createTniks(self.zclms, Z, self.sects[s], ii=c, why=why, v=1):
            c2 = c + nc*(s + ns*(l + nl*p))
            self.log(f'j={C} {JTEXTS[C]:4} {c=} {c2=}  lc={len(self.zclms)} plsc ={self.fplsc()} J1={self.fmtJ1(0, 1)} J2={self.fmtJ2(0, 1)} {z1=} {z2=}', file=1)
            for t, _ in enumerate(self.g_createTniks(self.snums, T, zc, why=why, v=1)):
                tlist, j, kk, txt = self.tnikInfo(*self.j2plsct(), z=1)
                self.log(f'{t=} {j=} {JTEXTS[j]:4} {c=} {c2=} ltl={len(tlist)} plsct={self.fplsct()} J1={self.fmtJ1(0, 1)} J2={self.fmtJ2(0, 1)} {z1=} {z2=}', file=1)
...
#            self.dumpTnik(self.colms[c3], C, why2)
        ...
        self.log(f'{zz=} j={C} {JTEXTS[C]:4} {nc=}       ltl={len(tlist)} plsc =[{self.fplsc()} J1={self.fmtJ1(0, 1)} J2={self.fmtJ2(0, 1)} z1={self.z1(c)} z2={self.z2(c)}', file=1)
        for t in range(nt):
            t2 = t + nt*(c + nc*(s + ns*(l + nl*p))) if j <= K else t
#                self.setJdump(j2, t, why2)
            self.setJ(j, t2)
            self.dumpTnik(tlist[t2], j, why)
            self.log(f'{zz=} {j=} {JTEXTS[j]:4} {nc=} {t2=} ltl={len(tlist)} plsct=[{p} {l} {s} {c} {t}] J1={self.fmtJ1(0, 1)} J2={self.fmtJ2(0, 1)}', file=1)
    ####################################################################################################################################################################################################
...
#        rb = b2
        ...
        rt = t2 % nt
        return rp, rl, rc, rt
    def j2plsct(self, p=None, l=None, s=None, c=None, t=None):
...
#        if self.VIEWS: # _, _, x, y, w, h = self.geom2(V, n=1, i=1, dbg=1)  ;  kk = 0  ;  kl = self.k[V] # view = self.createTnik(self.views, 0, V, x, y, w, h, kk, kl, why=why, dbg=1)
    ...
    def g_createTniks(self, tlist, j, pt, ii=None, why='', v=0, dbg=1, dbg2=1):
        ...
        n, _, x, y, w, h = self.geom(j, pt, n, ii, dbg=dbg2)   ;   text = ''   ;    kl = self.k[j]    ;  tlist2 = tlist   ;   p, l, c, _ = self.j2plct()
...
#            tnik = self.resizeZZs(tnik, why)
    ...
    def cc2xywh(self, dbg=1):
        ...
        if dbg: self.log(f'{cc=:3} {self.ftxywh(t)} {t.text=} {cw=} {ch=} {ca}', file=1)
...
#    def cn2plct(self, cn):
    ...
    def cn2txt(self, cn):  #  usefull? re-name f cn2tabtxt()
        cc = self.cn2cc(cn)
        p, l, c, t = self.cc2plct(cc)
        txt = self.data[p][l][c]
    ...
    def on_mouse_release(self, x, y, button, modifiers, dbg=1):
        np, nl, ns, nc, nt = self.n   ;   nz = self.zzl()    ;  nc += nz    ;   ll = self.LL
        ...
        if dbg: self.log(f'BGN button={button} modifiers={modifiers} txt={self.tabls[self.cc].text}', file=1)
...
#                self.moveTo(how, p0, l0, c0, t0)  ;  cc = self.cursorCol()  ;  self.log(f'AFT {cc0=} {p0=} {l0=} {c0=} {t0=} {cc=}')
    ...
    ####################################################################################################################################################################################################
    def togglePage(self, how, dp=1, dbg=1): #        self.dumpGeom('BGN', f'{how} {dp=:2} {self.i[P]=}')#        self.dumpGeom('END', f'{how} {dp=:2} {self.i[P]=}')
        self.dumpTniksPfx(how)
        self.toggleVisible(self.j()[P])
        self.dumpVisible()
        self.dumpVisible2()
        self.i[P] = 1 + (self.j()[P] + dp) % self.n[P]
        self.toggleVisible(self.j()[P])
        self.dumpVisible()
        self.dumpVisible2()
        self.dumpTniksSfx(how)
    ...
    def toggleVisible(self, p=None, dbg=1):
        why = 'TVis'   ;   J2 = self.J2   ;   np, nl, ns, nc, nt = self.n   ;   p0 = p   ;   i, i2 = 0, 0   ;   text = None
        p = p if p is not None else self.j()[P]
        self.log(f'BGN {why} {p=} {p0=} {J2[P]} pages[{p}].v={int(self.pages[p].visible)} j={util.fmtl(self.j())} i={util.fmtl(self.i)} {self.fVis()}')
        self.pages[p].visible = not self.pages[p].visible   ;   self.setJdump(P, p, why)
        for l in range(nl): # l2 = l + p*nl
            self.lines[J2[L]].visible = not self.lines[J2[L]].visible   ;   self.setJdump(L, l, why)  ;   vl = []
            for s0, s in enumerate(self.ss2sl()): # s2 = s0 + l*ns
                self.sects[J2[S]].visible = not self.sects[J2[S]].visible   ;   self.setJdump(S, s, why)
                for c in range(nc): # c2 = c + s*nc
                    self.colms[J2[C]].visible = not self.colms[J2[C]].visible   ;   self.setJdump(C, c, why)
                    for t in range(nt): # t2 = t + c*nt + s*self.tps + l*self.tpl # + p*self.tpp
                        imap = self.getImap(p, l, c) if s >= II else []
                        ...
                        elif s == II:                     text = self.imap2ikey(tobj, imap, i2, j);   i2 += 1 if text != self.tblank else 0
                        elif s == KK:                     text = self.imap2Chord(tobj, imap, t, j)
                        tnik = tlist[J2[j]]   ;   tnik.visible = not tnik.visible   ;   self.setJdump(j, t, why)   ;   v = int(tnik.visible)
                        if dbg:    self.log(f'{v=} {text=} {s0=} plsct={self.fplsct(p, l, s, c, t)}', file=1)
                        if dbg:                     vl.append(f'{v}')
                if dbg:                             vl.append(' ')
            if dbg:                                 self.log(f'{"".join(vl)}', pfx=0)
        self.log(f'END {why} {p=} {p0=} {J2[P]} pages[{p}].v={int(self.pages[p].visible)} j={util.fmtl(self.j())} i={util.fmtl(self.i)} {self.fVis()}')

    def OLD__toggleVisible(self, p=None, dbg=1):
        why = 'TVis'   ;   J2 = self.J2   ;   np, nl, ns, nc, nt = self.n   ;   p0 = p   ;   i, i2 = 0, 0   ;   text = None
        p = p if p is not None else self.j()[P] #  ;   page = self.pages[p]
        self.log(f'BGN {why} {p=} {p0=} {J2[P]} pages[{p}].v={int(self.pages[p].visible)} j={util.fmtl(self.j())} i={util.fmtl(self.i)} {self.fVis()}')
        self.setJdump(P, p, why)                          ;   self.pages[p].visible = not self.pages[p].visible
        for l in range(nl): # l2 = l + p*nl
            self.setJdump(L, l, why)                      ;   self.lines[J2[L]].visible = not self.lines[J2[L]].visible   ;   vl = []
            for s0, s in enumerate(self.ss2sl()): # s2 = s0 + l*ns
                self.setJdump(S, s, why)                  ;   self.sects[J2[S]].visible = not self.sects[J2[S]].visible
                for c in range(nc): # c2 = c + s*nc
                    self.setJdump(C, c, why)              ;   self.colms[J2[C]].visible = not self.colms[J2[C]].visible
                    for t in range(nt): # t2 = t + c*nt + s*self.tps + l*self.tpl # + p*self.tpp
#                        s = self.ss2sl()[self.J1[S]]
                        imap = self.getImap(p, l, c) if s >= II else []
                        tlist, j, kl, tobj = self.tnikInfo(p, l, s, c, t, why=why)
                        ...
                        tnik = tlist[J2[j]]   ;   tnik.visible = not tnik.visible   ;   self.setJdump(j, t, why)   ;   v = int(tnik.visible)
                        if dbg:    self.log(f'{v=} {text=} {s0=} plsct={self.fplsct(p, l, s, c, t)}', file=1) # plsct2={self.fplsct(p, l2, s2, c2, t2)} and self.VERBOSE
                        if dbg:                     vl.append(f'{v}')
                if dbg:                             vl.append(' ')
            if dbg:                                 self.log(f'{"".join(vl)}', pfx=0)
        self.log(f'END {why} {p=} {p0=} {J2[P]} pages[{p}].v={int(self.pages[p].visible)} j={util.fmtl(self.j())} i={util.fmtl(self.i)} {self.fVis()}')
...
#        else:               file = LOG_FILE  ;  so = 1
        if   file is None: file = LOG_FILE if not so else sys.stdout
    ...
    def log(self, msg='', pfx=1, pos=0, file=None, flush=False, sep=',', end='\n'):
        so = 0 if file is None or 1 else 1
        file = sys.stdout if file == 1 else LOG_FILE
        if   pos: msg = f'{self.fmtPos()}' + f' {msg}'
        util.slog(msg, pfx, file, flush, sep, end, so)
    ####################################################################################################################################################################################################
...
########################################################################################################################################################################################################
def initRGB(dbg=1):
    if dbg: s = f'{B*7}'  ;  t = f'{s}RGB '  ;  o = [ f' {o}' for o in range(len(OPC)) ]  ;  util.slog(f'RGB{s}{util.fmtl(o, w="3",d1="")}{t}Diffs  {t}Steps', pfx=0, file=LOG_FILE)
