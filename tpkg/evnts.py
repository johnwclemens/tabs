...
def on_key_press(tobj, symb, mods, dbg=1):
    ...
    elif kbk == 'G' and isCtl(   kd, m):     cmd = cmds.Move2FirstTabCmd(tobj, '@ G', page=0)  ;  cmd.do()
    elif kbk == 'H' and isCtlShf(kd, m):     cmd = cmds.Move2LastTabCmd(tobj,  '@^H', page=1)  ;  cmd.do()
    elif kbk == 'H' and isCtl(   kd, m):     cmd = cmds.Move2LastTabCmd(tobj,  '@ H', page=0)  ;  cmd.do()
