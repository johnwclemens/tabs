...
#   elif kbk == 'E' and isCtl(   kd, m):     tobj.eraseTabs(    '@ E')
    ...
    elif kbk == 'O' and isCtlShf(kd, m):     cmd = cmds.TogCrsrModeCmd(tobj, '@^O', -1)       ;  cmd.do()
    elif kbk == 'O' and isCtl(   kd, m):     cmd = cmds.TogCrsrModeCmd(tobj, '@ O',  1)       ;  cmd.do()
...
#   elif kbk == 'BACKSLASH':                  tobj.setCHVMode(  '  BACKSLASH', ARPG,   RARROW, DARROW)
    ...
    ####################################################################################################################################################################################################
    elif kbk == 'B' and isAltShf(kd, m):     cmd = cmds.SetFontParamCmd(tobj, BOLD,      not fontBold,   'fontBold')     ;  cmd.do()
    elif kbk == 'B' and isAlt(   kd, m):     cmd = cmds.SetFontParamCmd(tobj, BOLD,      not fontBold,   'fontBold')     ;  cmd.do()
    elif kbk == 'C' and isAltShf(kd, m):     cmd = cmds.SetFontParamCmd(tobj, COLOR,        -1,          'clrIdx')       ;  cmd.do()
    elif kbk == 'C' and isAlt(   kd, m):     cmd = cmds.SetFontParamCmd(tobj, COLOR,         1,          'clrIdx')       ;  cmd.do()
    elif kbk == 'I' and isAltShf(kd, m):     cmd = cmds.SetFontParamCmd(tobj, ITALIC,    not fontItalic, 'fontItalic')   ;  cmd.do()
    elif kbk == 'I' and isAlt(   kd, m):     cmd = cmds.SetFontParamCmd(tobj, ITALIC,    not fontItalic, 'fontItalic')   ;  cmd.do()
    elif kbk == 'M' and isAltShf(kd, m):     cmd = cmds.SetFontParamCmd(tobj, FONT_NAME,    -1,          'fontNameIdx')  ;  cmd.do()
    elif kbk == 'M' and isAlt(   kd, m):     cmd = cmds.SetFontParamCmd(tobj, FONT_NAME,     1,          'fontNameIdx')  ;  cmd.do()
    elif kbk == 'S' and isAltShf(kd, m):     cmd = cmds.SetFontParamCmd(tobj, FONT_SIZE,     33 / 32,    'fontSize')     ;  cmd.do()
    elif kbk == 'S' and isAlt(   kd, m):     cmd = cmds.SetFontParamCmd(tobj, FONT_SIZE,     32 / 33,    'fontSize')     ;  cmd.do()
