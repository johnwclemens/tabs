...
#        if not self.useOrderedGroup: self.rootGroup = pyglet.graphics.Group()
    ...
    @staticmethod
    def printSpriteInfo_OLD(reason, p, l, r, c, x, y, w, h, xm, ym, ww, hh, ii, jj, visible, color):
    ...
    @staticmethod
    def printSpriteInfo(reason, s):
        f = '{:6.1f} {:6.1f} {:6.1f} {:6.1f} {:5.3f} {:5.3f} {:5.3f} {:5.1f} {} {} {}'
        fs = f.format(s.x, s.y, s.width, s.height, s.scale, s.scale_x, s.scale_y, s.rotation, s.color, s.opacity, s.visible)
        print('{:32} {}'.format(reason, fs), file=DBG_FILE)
...
#        w, h = ww, hh
        ...
        for p in range(ppf):
            ...
            self.printSpriteInfo('_initPages({:6.1f} {:6.1f} {:6.1f} {:6.1f} {} {}) {:2} {:2} {:2} {:3}'.format(xx, yy, ww, hh, ii, jj, p, l, r, c), page)
...
#            self.pages.append(lines)
    ...
    def _initLines(self, xx, yy, ww, hh, p=-1, ii=6, jj=6):
        ...
        for l in range(lpp):
            ...
            self.printSpriteInfo('_initLines({:6.1f} {:6.1f} {:6.1f} {:6.1f} {} {}) {:2} {:2} {:2} {:3}'.format(xx, yy, ww, hh, ii, jj, p, l, r, c), line)
...
#                self._lines.append(self.lines[l])
    ...
    def _initRows(self, xx, yy, ww, hh, p=-1, l=-1, ii=4, jj=4):
        ...
        for r in range(rpl):
            ...
            self.printSpriteInfo('_initRows() {:2} {:2} {:2} {:3}'.format(p, l, r, c), row)
...
#        lines.append(rows)
    ...
    def _initCols(self, xx, yy, ww, hh, p=-1, l=-1, r=-1, ii=2, jj=2):
        ...
        for c in range(self.colsPerRow):
            ...
            self.printSpriteInfo('_initCols() {:2} {:2} {:2} {:3}'.format(p, l, r, c), col)
...
#        rows.append(cols)
    ...
    def on_resize(self, width, height):
        ...
        for p in range(np):
            self.printSpriteInfo('on_resize(BEFORE) pages {:2}'.format(p), self.pages[p])
            self.pages[p].update(x=self.pages[p].x*mx, y=self.pages[p].y*my, scale_x=mx, scale_y=my)
            self.printSpriteInfo('on_resize(AFTER) pages {:2}'.format(p), self.pages[p])
        for l in range(nl):
            self.printSpriteInfo('on_resize(BEFORE) lines {:2}'.format(l), self.lines[l])
            ii, jj = 6, 6
            w, h = ww-2*ii, (hh-(lpp+1)*jj)/lpp
...
#            self.lines[l].update(x=self.lines[l].x*mx, y=self.lines[l].y*my, scale_x=mx, scale_y=my)
            ...
            self.printSpriteInfo('on_resize(AFTER) lines {:2}'.format(l), self.lines[l])
