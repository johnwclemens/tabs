...
#    hideST2 = ['log', 'plct2cc']  # , '<listcomp>', 'dumpImap']
    hideST  = ['log', 'dumpGeom', 'dumpJ', 'dumpData', 'dumpSelectTabs', 'cc2plct', 'cursorCol', 'dumpCursorArrows', 'setCaption', '<listcomp>', 'dumpImap', 'objs2Text']
    def __init__(self):
        dumpGlobals()
...
#            self.push_handlers(self.keyboard)
    ...
    def fPos(self):  plct = self.j2()   ;   cc = self.plct2cc(*plct)   ;   plct.append(cc)   ;   plct.append(self.cc2cn(cc))  ;   return f'{fmtl(plct)}'
#    def fPos(self):  plct = self.j2()   ;   cc = self.plct2cc(*plct)   ;   return f'{fmtl(plct)} {cc} {self.cc2cn(cc)}'
    ...
    @staticmethod
    def dumpObjs(objs, name, why=''): [Tabs.dumpObj(o, name, why) for o in objs]  # ;   [Tabs.slog(f'{hex(id(o))} type={type(o)}', ind=0) for o in obj]   ;    Tabs.slog(ind=0)
...
#    def dumpS(self): self.log(f'{self.ss()} {fmtl(self.TNIK)} {fmtl(self.n)} {self.J1[S]} {self.J2[S]}')
...
#        self.snapshot()
        ...
        if dbg: self.log(f'END {self.fmtWH()} {fmtl(self.n)} {(fmtl(self.TNIK))} {self.fmtDD(self.data)} {fmtl(self.lenE()[:-2])} {sum(self.lenE()[:-2])}')
    ####################################################################################################################################################################################################
...
#        self.dumpData(why)
    ...
    def dumpStruct2(self, why=''):
        ...
        self.dumpGlobalFlags()
        self.dumpGeom('   ')
        self.log(f'{self.fmtDD(self.data)} {self.fmtDD(self.data)}')
    ...
    def readDataFile(self, dbg=0):  #  empty file bug
...
#        self.log(f'END {why}')
    ...
    def dumpDataHorz(self, data=None, lc=1, ll=1, i=0):
        if data is None: data = self.data
        self.log(f'BGN lc={lc} ll={ll} i={i} {self.fmtDD(data)}')
        for p in range(len(data)):
            for l in range(len(data[p])):
                ...
                for r in range(len(data[p][l])):
                    ...
                    self.log(ind=0)
                self.log(ind=0)
        self.log(f'END lc={lc} ll={ll} i={i} {self.fmtDD(data)}')
    ...
    def transposeData(self, data=None, why=' External  ', dbg=0):
        if data is None: data = self.data
        if dbg: self.log(f'BGN {why} {self.fmtDD(data)}')
        transposed = []
        for p in range(len(data)):
            lines = []
            for l in range(len(data[p])):
        ...
        if dbg: self.log(f'END {why} {self.fmtDD(transposed)}')
        return transposed
    ...
    def hideLabels(self, p, t, r1, r2=0, r3=1, dbg=1):
        for j in range(r1, r2, r3):
            ...
            if len(p) > j: self.log(f'{j+1:3} {r2:3} {r3:3} hide {text}', ind=0) if dbg else None   ;   self.hideLabel(p, j, t)
    ...
    def hideLabel(self, p, j, t='', dbg=1): #        c = self.E[j][n]
        ...
        if dbg: self.log(f'{t:5} {j+1:3} {hex(id(c))} {text:6} {x:7.2f} {y:7.2f} {w:7.2f} {h:7.2f} {c.x:7.2f} {c.y:7.2f} {c.width:7.2f} {c.height:7.2f}', ind=0)
    ####################################################################################################################################################################################################
    def toggleTabs(self, how, tnik):
#        if       self.TNIK[tnik] and not self.B[tnik]: msg = 'SHOW '   ;   self.showTabs(tnik)
#        elif     self.n[S]:                            msg = 'HIDE '   ;   self.hideTabs(tnik)
#        else:                                          msg = 'SKIP '   ;   self.dumpGeom('   ', msg)
        self.dumpGeom('BGN', how)
        if   not self.TNIK[tnik] and not self.B[tnik]: msg = 'SHOW '   ;   self.showTabs(tnik)
        elif     self.TNIK[tnik]:                      msg = 'HIDE '   ;   self.hideTabs(tnik)
        else:                                          msg = 'SKIP '   ;   self.dumpGeom('   ', msg)
#        self.toggleTnik(tnik, msg)
        self.on_resize(self.ww, self.hh, dbg=1)
        self.dumpGeom('END', msg)

    def hideTabs(self, tnik):
#        tabs = self.B[tnik]  ;  stnik = self.ss()
        msg = f'HIDE tnik={tnik}'
        ttype = 'Tab' if tnik == TT  else 'Note' if tnik == NN else 'Ikey' if tnik == II else 'Chord' if tnik == KK else '???'
        np, nl, ns, nc, nt = self.n
        self.dumpGeom('BGN', msg) # both labels and tabs, notes, sects and chords?
        self.toggleTnik(tnik, msg)
        for l in range(nl):
            self.hideLabel(self.sects, l * ns + tnik, 'Sect', dbg=1)
            for c in range(nc):
...
#            self.hideLabel(tabs, t, ttype, dbg=dbg)
    ...
    def OLD2_showTabs(self, tnik, dbg=1):
        tabs = self.B[tnik]   ;   tnikB = tnik + len(self.B)  # ;   stnik = self.ss()
#        ttype = 'Tab' if tnik == TT  else 'Note' if tnik == NN else 'Chord' if tnik == KK else '???'
        ks  = self.ks       ;   kc = self.kc  ;   msg = f'SHOW tnik={tnik}'
        k = [self.kt, self.kn, self.ki, self.kk]
        self.dumpGeom('BGN', msg)
        for i in range(self.n[L]):
            self.showLabel(i, self.sects, j=S, g=self.g[S], k=ks, dbg=dbg)
            for j in range(self.n[C]):
                self.showLabel(j, self.cols, j=C, g=self.g[C], k=kc, dbg=dbg)
                for kk in range(self.n[T]):
                    self.showLabel(kk, tabs, j=tnikB, g=self.g[T], k=k[tnik], dbg=dbg)
        self.dumpGeom('END', msg)
    def OLD_showTabs(self, tnik, dbg=1, dbg2=0):
...
#        if SNAP and dbg: self.snapshot(f'showTabs() {fmtl(self.TNIK)} {msg}')
    ...
    def toggleRLCols(self, how):
        ...
        if show: msg = 'SHOW '   ;   self.showRLCols()
        else:    msg = 'HIDE '   ;   self.hideRLCols()
        self.dumpGeom('RSZ', msg)
        self.on_resize(self.ww, self.hh, dbg=1)
        self.dumpGeom('END', msg)
...
#        cc = 2 # if CCC else 1
    ...
    def toggleLLRows(self, how):
        ...
        self.dumpGeom('BGN', how)
        self.log(f'BGN old QQ={old} new QQ={QQ} CCC={CCC}')  ;  self.log(f'llText={fmtl(self.llText[CCC])}')
        if QQ and not self.lrows: self.showLLRows()
        else:                     self.hideLLRows()
        self.log(f'    old QQ={old} new QQ={QQ} CCC={CCC}')  ;  self.log(f'llText={fmtl(self.llText[CCC])}')
        self.on_resize(self.ww, self.hh, dbg=1)
        self.log(f'END old QQ={old} new QQ={QQ} CCC={CCC}')  ;  self.log(f'llText={fmtl(self.llText[CCC])}')
        self.dumpGeom('END', how)
...
#        elif type(p) is pyglet.sprite.Sprite: my = (p.height - hr)/self.h[L]   ;   p.scale_y = my   ;   self.log(f'ntl={ntl:2} h[L]={self.h[L]} my={my:7.5f} p.y={p.y:7.2f} p.h={p.height:7.2f}', ind=0)
    def createLLRow(self, p, pi, dbg=1, dbg2=0):
        ...
        if dbg: self.log(f'pi={pi:3} px={p.x:7.2f} py={p.y:7.2f} pw={p.width:7.2f} ph={p.height:7.2f} nn1={nn1} nn2={nn2}', ind=0)
        if dbg: self.log(f'nr={nr:3} xr={xr:7.2f} yr={yr:7.2f} wr={wr:7.2f} hr={hr:7.2f} row.y={row.y:7.2f} row.h={row.height:7.2f}', ind=0)
        if dbg: self.log(f'nc={nc:3} xc={xc:7.2f} yc={yc:7.2f} wc={wc:7.2f} hc={hc:7.2f} sc={sc}', ind=0)
        if   type(p) is pyglet.text.Label:    p.y, p.height = self.squeeze(p.y, p.height, 1 + self.n[T]*self.ss()*self.i[L])   ;   self.log(f'nn1={nn1:2} nn2={nn2:2} p.y = {p.y:7.2f} p.h = {p.height:7.2f}', ind=0) if dbg else None
        elif type(p) is pyglet.sprite.Sprite: my = (p.height - hr)/self.h[L]   ;   p.scale_y = my    ;   self.log(f'nn1={nn1:2} nn2={nn2:2} h[L]={self.h[L]} my={my:7.5f} p.y={p.y:7.2f} p.h={p.height:7.2f}', ind=0) if dbg else None
        return p
    ...
    def resizeLLRow(self, p, pi, dbg=0, dbg2=0):
        ...
        if dbg: self.log(f'nc={nc:3} xc={xc:7.2f} yc={yc:7.2f} wc={wc:7.2f} hc={hc:7.2f} sc={sc}', ind=0)
        if   type(p) is pyglet.text.Label:    p.y, p.height = self.squeeze(p.y, p.height, 1 + self.n[T]*self.ss()*self.i[L])   ;   self.log(f'nn1={nn1:2} nn2={nn2:2} p.y = {p.y:7.2f} p.h = {p.height:7.2f}', ind=0) if dbg else None
        elif type(p) is pyglet.sprite.Sprite: my = (p.height - hr)/self.h[L]   ;   p.scale_y = my   ;      self.log(f'nn1={nn1:2} nn2={nn2} h[L]={self.h[L]} my={my:7.5f} p.y={p.y:7.2f} p.h={p.height:7.2f}', ind=0) if dbg else None
        return p
#        if   type(p) is pyglet.text.Label:    p.y -= hr/2   ;   p.height *= ntl / (ntl + 1) / j1l   ;   self.log(f'ntl={ntl:2} p.y = {p.y:7.2f} p.h = {p.height:7.2f}', ind=0)
#        elif type(p) is pyglet.sprite.Sprite: my = (p.height - hr)/self.h[L]   ;   p.scale_y = my   ;   self.log(f'ntl={ntl:2} h[L]={self.h[L]} my={my:7.5f} p.y={p.y:7.2f} p.h={p.height:7.2f}', ind=0)
#        if dbg2: self.log(f'p.y -= hr/2 = {p.y:7.2f}, p.height -= h = {p.height:7.2f}  w={fmtl(self.w)} h={fmtl(self.h)}', ind=0)
    ...
    def geom(self, p=None, j=0, init=0, nn=0, dbg=0):
        ...
        if   n == 0:            n = 1        ;     dbz = 1   ;  msg  = f'WARN j={j} setting n=0 -> n=1 -> n=0'
        if   nn:                n = nn
        if   j == C:            n += CCC     #    if   j >= T:            n += QQ
        ...
        if   dbz:               n = 0                        ;  self.log(msg, ind=0)
        if   dbz or dbg: self._dumpGeom(j, n, i, x, y, w, h, mx, my, iw, ih, nn, p)
...
#        self.log(f'  {j}  {n:3} {i:4}   {x:7.2f} {y:7.2f} {w:7.2f} {h:7.2f} {mx:6.3f} {my:6.3f}  {iw:7.2f} {ih:7.2f} {nn:3} {JTEXTS[j]} {px:7.2f} {py:7.2f} {pw:7.2f} {ph:7.2f}', ind=0, file=sys.stdout)
    ...
    def toggleTnik(self, tnik, msg):
#        msg += f' tnik={tnik}'
        self.dumpGeom('BFR', msg)
        self.TNIK[tnik] = not self.TNIK[tnik]    ;    self.n[S] = self.ss()
        self.dumpGeom('AFT', msg)

    def OLD3_showTabs(self, tnik):
        msg = f'SHOW'   ;   c = self.n[C]
        self.dumpGeom('BGN', msg)
        self.toggleTnik(tnik, msg)    ;    self.J1[S], self.J2[S] = 0, 0   ;   self.J1[L], self.J2[L] = 0, 0
        self.dumpJ(msg)  # ;   self.initJ(msg)
        for i, line in enumerate(self.lines):
            self.J1[S] += 1   ;   self.J2[S] += 1    ;   self.J1[L] = i    ;   self.J2[L] = i
            for sect in      self.g_createLabels(line, S, self.sects, nn=1):
                for col in   self.g_createLabels(sect, C, self.cols,  nn=c):
                    for _ in self.g_createLabels2(col, 1):
                       pass  # self.J1[L] += 1  # ;   self.J2[L] += 1
        self.dumpJ(msg)
        self.dumpGeom('END', msg)
...
#            self.J1[L] = i
#            for sect in      self.g_createLabels(line, S, self.sects): # , nn=1):
#                for col in   self.g_createLabels(sect, C, self.cols): # ,  nn=self.n[C]):
#                    for _ in self.g_createLabels2(col):
#                        pass
        self.dumpJ(msg)
        self.dumpGeom('END', msg)
    ####################################################################################################################################################################################################
    def createLabels(self):
        self.dumpGeom('BGN')
        self.dumpLabel()
        for page in              self.g_createLabels(None, P, self.pages):
            for line in          self.g_createLabels(page, L, self.lines):
                for sect in      self.g_createLabels(line, S, self.sects):
                    for col in   self.g_createLabels(sect, C, self.cols):
                        for _ in self.g_createLabels2(col):
                            pass
        self.dumpLabel()
        self.dumpGeom('END')

    def g_createLabels(self, p, j, lablist, nn=0, dbg=1, dbg2=0):
        ...
        for tt in range(n):
            ...
            if QQ and j == L: self.createLLRow(lbl, tt)
            yield lbl
    ####################################################################################################################################################################################################
...
#            else: msg = f'ERROR tti={self.tti()} s={s} n={fmtl(self.n)} {fmtl(self.TNIK)} {self.ss()} tt nn kk={int(tt)} {int(nn)} {int(kk)} {fmtl(self.lenE())}'  ;  self.log(msg)  ;  self.quit(msg) #  yield None
    ...
    def resizeLabels(self, dbg=0):
        ...
        for page in              self.g_resizeLabels(None, P, self.pages, why=JTEXTS[P]):
            for line in          self.g_resizeLabels(page, L, self.lines, why=JTEXTS[L]):
                for sect in      self.g_resizeLabels(line, S, self.sects, why=JTEXTS[S]):
                    for col in   self.g_resizeLabels(sect, C, self.cols,  why=JTEXTS[C]):
                        for _ in self.g_resizeLabels2(col):
                            pass
        if dbg: self.dumpLabel()
        self.dumpGeom('END')

    def g_resizeLabels(self, p, j, lablist, why, dbg=0, dbg2=0):#        if j >= T: n += QQ   ;   self.log(f'j={j} n={n} ')
        ...
        for tt in range(n):
            ...
            yield lbl
    ####################################################################################################################################################################################################
    def g_resizeLabels2(self, col, dbg=0, dbg2=0):
        stnik = self.ss()
...
#            elif nn and (s == 1 or (s < 1 and not tt)):
#            elif nn and (s == 1 or (s == 0 and not tt)):
            elif self.tt0(s) == NN:
                ...
                if dbg:   self.dumpLabel(lbl, *self.ids(), *self.cnts(), why=why)
#            elif ii and (s == 2 or (s < 2 and (not tt and not nn))):
#            elif ii and (s == 2 or (s == 1 and (not tt or not nn)) or (s == 0 and (not tt and not nn))):
            elif self.tt0(s) == II:
                ...
                else:                      ikey = self.ikeys[si]      ;     si += 1   ;  why += f'IKey {si}'
...
#            elif kk and (s == 3 or (s == 2 and (not tt or not nn or not ii)) or (s == 1 and (tt + nn + ii == 2)) or (s == 0 and (not tt and not nn and not ii))):
            elif self.tt0(s) == KK:
            ...
            else:         self.log(f'ERROR s={s} {fmtl(self.TNIK)} {stnik} [{tt} {nn} {ii} {kk}] {fmtl(self.lenE())}') #  ;  self.log(msg)  # ;  self.quit(msg)
            yield lbl
    ####################################################################################################################################################################################################
...
#        if len(p) > j2[j]:   self.log(f'ERROR Label Exists? len(p)={len(p)} j={j} j2[j]={j2[j]}')   # ;  self.quit('ERROR Unexpected if')
    ...
    def NEW_dumpLabel(self, b=None, _=-1, lid=-1, p=-1, l=-1, s=-1, c=-1, t=-1, n=-1, i=-1, k=-1, o=-1, a=-1, d=-1, e=-1, f=-1, g=-1, r=-1, z=-1, why=''):
        if b is None: self.log(f'P L S C T lid p  l  s   c   t   n   i   k   z    x       y       w       h    text      identity  siz dpi b i red grn blu opc   why', ind=0) ; return
...
#    def cn2plct(self, cn):
    ...
    def on_key_press(self, symb, mods, dbg=0): # avoid these
        ...
        elif kbk == 'R' and self.isCtrl(     mods):    self.toggleChordNames('@   R', rev=1)
...
#        self.armSnap = f'move() k={k:4} kk={kk:4} {fmtl(self.i, FMTN)} text={self.tabs[self.cc].text} {x:6.2f} {y:6.2f}'
    ...
    def unselectTabs(self, how, m, k=None, dbg=VERBOSE):
        ...
        if k in self.smap: self.smap.pop(k)  #  crashes?  remove(k)
    ...
    def pasteTabs(self, how, kk=0, dbg=VERBOSE):
        ...
        for i, (k, v) in enumerate(self.smap.items()):
            ...
            for t in range(nt):
                ...
                self.setDTNIK(text[t], kt, p, l, c, t, uk=1 if t == nt - 1 else 0)  # call with uk=1 only once per column or tpc
...
#        self.dumpSelectTabs(f'END {how} hc={hc} kk={kk} cc={cc} ntc={ntc} cn={self.cc2cn(cc) + 1}')
    ...
    def setDTNIK(self, text, cc, p, l, c, t, uk=0, dbg=1):  # what was previous data value
        if dbg: self.log(f'BGN uk={uk}    text={text}', pos=1)
        ...
        if dbg: self.log(f'END uk={uk}    text={text}', pos=1)
    ####################################################################################################################################################################################################
    def setData(self, text, p, l, c, t, dbg=1):
        data = self.data[p][l][c]   ;   action = 'set'
        if dbg: self.log(f'BGN {action} t={t} text={text} data={data}', pos=1)
        ...
        if dbg: self.log(f'END {action} t={t} text={text} data={data}', pos=1)

    def setTab2(self, text, cc, dbg=1):
#        cc = self.plct2cc(*self.j2())
        if dbg: self.log(f'BGN         text={text} tabs[{cc}]={self.tabs[cc].text}', pos=1)
        self.tabs[cc].text = text
        if dbg: self.log(f'END         text={text} tabs[{cc}]={self.tabs[cc].text}', pos=1)

    def setNote(self, text, cc, t, dbg=1):
#        p, l, c, t = self.j2()   ;   cc = self.plct2cc(p, l, c, t)
        if dbg: self.log(f'BGN     t={t} text={text} notes[{cc}]={self.notes[cc].text}', pos=1)
        self.notes[cc].text = self.getNoteName(t, text) if self.isFret(text) else self.nblank
        if dbg: self.log(f'END     t={t} text={text} notes[{cc}]={self.notes[cc].text}', pos=1)
...
#        else:          self.log(f'{NOT FOUND key={key} keys={fmtl(list(mli.keys()))}', pos=1) if dbg else None   ;   imap = self.cobj.getChordName(p, l, c)  # ;   mli[key] = imap
        self.log(f'key={key} keys={fmtl(list(mli.keys()))}') if dbg else None   ;   imap = self.cobj.getChordName(p, l, c)  # ;   mli[key] = imap
...
#        p, l, c, t = self.j2()
        ...
        if dbg: self.log(f'BGN ikeys={fmtl(ikeys)}', pos=1)
        self.setIkeyText(ikeys, cc, p, l, c)
        if dbg: self.log(f'END ikeys={fmtl(ikeys)}', pos=1)
    ...
    def setIkeyText(self, text, cc, p, l, c, dbg=0):
        ...
        self.log(f'BGN [{cc:2}-{cc+nt-1:2}] text={fmtl(text)} data={data} ikeys=<{txt}>{len(txt)}', pos=1)
        for i in range(nt):
        ...
        txt = self.objs2Text(self.ikeys, cc, nt, I)
        self.log(f'END [{cc:2}-{cc+nt-1:2}] text={fmtl(text)} data={data} ikeys=<{txt}>{len(txt)}', pos=1)
...
#        p, l, c, t = self.j2()
        ...
        if dbg: self.log(f'BGN name={name} chunks={fmtl(chunks)}', pos=1)
        self.setChordName(cc, name, chunks)
        if dbg: self.log(f'END name={name} chunks={fmtl(chunks)}', pos=1)

    def setChordName(self, cc, name, chunks):
        nt = self.n[T]   ;   cc = self.normalizeCC(cc)   ;   chords = self.chords
        txt = self.objs2Text(chords, cc, nt, K)
        self.log(f'BGN [{cc:2}-{cc+nt-1:2}] name={name} chunks={fmtl(chunks)} chords=<{txt}>{len(txt)}', pos=1)
        for i in range(nt):
            if chunks and len(chunks) > i: self.chords[cc + i].text = chunks[i]
            else:                          self.chords[cc + i].text = self.nblank
        txt = self.objs2Text(chords, cc, nt, K)
        self.log(f'END [{cc:2}-{cc+nt-1:2}] name={name} chunks={fmtl(chunks)} chords=<{txt}>{len(txt)}', pos=1)
    @staticmethod
    ...
    def imapKeys2matches(self, iks, rank, dbg=1):
...
#        if dbg: self.dumpSelectTabs(f'BGN {how} rev={rev} key={key}       ')
        if dbg: self.log(f'BGN {how} rev={rev} key={key}', pos=1)
        ...
        if dbg: self.cobj.dumpImap(imap, f'{self.fPos()}')
...
#        if dbg: self.log(f'END {how} rev={rev} key={key}', pos=1)
        if dbg: self.log(f'END {how} rev={rev} key={key} cc={cc:3}', pos=1)
    ####################################################################################################################################################################################################
...
#    def isBTab(self, text):   return 1 if text == self.tblank else 0
    ...
    def getFilePath(seq=0, filedir='files', filesfx='.txt'):
        if seq:
            ...
            Tabs.slog(f'{fmtl(fileGlob)}', ind=0)
    ...
    def log(self, msg='', ind=1, pos=0, file=None, flush=False, sep=',', end='\n'):
        if pos: msg = f'{self.fPos()} ' + msg
        self.slog(msg, ind, file, flush, sep, end)
        
    @staticmethod
    def slog(msg='', ind=1, file=None, flush=False, sep=',', end='\n'):
...
#        if file != LOG_FILE: Tabs.slog(msg, ind, flush=False, sep=',', end=end)
    ####################################################################################################################################################################################################
